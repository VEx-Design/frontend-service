    ...
    <div className="flex flex-col h-full w-full border rounded-lg overflow-hidden bg-background">
      <div className="p-2 border-b flex items-center justify-between bg-muted/30">
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  ...
                  className="h-8 w-8"
                  onClick={() =>
                    setAction(action === "move" ? "select" : "move")
                  }
                >
              ...
              <TooltipContent>
                {action === "move" ? "Switch to Select" : "Switch to Move"}
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </div>
        ...
        <div className="flex items-center gap-4 text-xs text-muted-foreground">
          ...
          {squareSize.width > 0 && (
            <span>
              Size: {(squareSize.width / scalingFactor).toFixed(1)}mm ×{" "}
              {(squareSize.height / scalingFactor).toFixed(1)}
              mm
            </span>
          )}
        </div>
          ...
          <Layer>
            ...
            {showPoints &&
              ...
              (() => {
                ...
                return Array.from(combinedPoints.values()).map(
                  ({ x, y, labels }, index) => {
                    ...
                    return (
                      <React.Fragment key={`${x},${y}`}>
                        ...
                        <Text
                          text={`(${Math.round(
                            (x * squareSize.width) / scalingFactor
                          )}, ${Math.round(
                            (y * squareSize.height) / scalingFactor
                          )}) mm\n${labels.join(", ")}`}
